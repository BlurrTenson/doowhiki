...
local file_scratchpad = require("dooing.ui.file_scratchpad")
...
function M.create_help_window()
	...
	vim.keymap.set(
		...
		close_help,
		{ buffer = constants.help_buf_id, nowait = true }
	)
	vim.keymap.set(
		"n",
...
function M.open_todo_scratchpad()
	...
	local cfg = require("dooing.config").options
	if cfg.scratchpad and cfg.scratchpad.use_files then
		file_scratchpad.open(todo)
		return
	end
